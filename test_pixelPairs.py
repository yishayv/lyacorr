...
class TestPixelPairs(TestCase):
    def test_find_nearby_pixels(self):
        ...
        ar_z = np.arange(1.95, 3.56, 0.001)
        delta_t_file.set_wavelength(0, ar_z)
        delta_t_file.set_flux(0, np.sin(ar_z * 50))
        delta_t_file.set_ivar(0, ar_z)

        delta_t_file.set_wavelength(1, ar_z)
        delta_t_file.set_flux(1, np.sin(ar_z * 50))
        delta_t_file.set_ivar(1, ar_z)

        pixel_pairs = calc_pixel_pairs.PixelPairs(cd, radius)
        pixel_pairs.find_nearby_pixels(accumulator=pair_separation_bins_1, qso_angle=0.04, spec1_index=0, spec2_index=1,
                                       delta_t_file=delta_t_file)

        pixel_pairs.find_nearby_pixels(accumulator=pair_separation_bins_2, qso_angle=0.04, spec1_index=0, spec2_index=1,
                                       delta_t_file=delta_t_file)
